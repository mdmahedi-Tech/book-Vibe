...
const ReadList = ({sort}) => {
     const {markread}=useContext(bookContext)
            //console.log(markread,"read");

            const[filterdreadlist,setfilterdreadlist]=useState(markread)
            // shorrting function
            useEffect(()=>{
           if(sort){
            if(sort==='pages'){
          const shortData=[...markread].sort((a,b)=>a.totalPages-b.totalPages);
          console.log(shortData)
          setfilterdreadlist(shortData);
            
           }
           else if(sort==='rating'){
          const shortData=[...markread].sort((a,b)=>b.rating-a.rating);
          console.log(shortData)
          setfilterdreadlist(shortData);
            ...
            if(filterdreadlist.length===0 ){
                return <div className='flex justify-center items-center bg-red-200 h-100'>
                    <h1 className='text-6xl font-bold opacity-70'>Readable data no found here</h1></div>
            }
    return (
        <div className='my-y'>
    ...
    <div className="card-body col-span-3">
       ...
       <h2 className="card-title text-2xl font-bold">{book.bookName}</h2>
           ...
           <p className='flex text-xl justify-between items-center'> Rating:<span className='font-bold'>{book.rating}</span><FaRegStar /></p>
